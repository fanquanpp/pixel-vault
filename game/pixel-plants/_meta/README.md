...
| 类别 | 画布 |
|---|---|
| 全部类别（古典仙气 / 浪漫花语 / 本草清雅 / 树木佛意 / 野趣多肉） | 64×64 |

2026-09-17 起全部素材按 64×64 画布重新绘制：构图沿用 16px（树木 32px）调校期的
比例，绘制原语按比例因子（花草 S = w/16 = 4、树木 ST = w/32 = 2）缩放，
手工点缀的最小细节以 4px（树木 2px）块呈现。
...
单枚瓦片不透明颜色数控制在 4–7（实测 3–8），符合 txt「每株限制 4–6 色」的意图。
...
- 帧条尺寸：64×64 素材为 256×64。
- 逐帧位移：`bloom` / `core` / `glint` 横向 ±S px（S = 画布/16，64px 时为 4），
  `leaf` ±S/2 px（取整），`shadow` 与 `stem` 不动，形成「花头轻摆」效果。
